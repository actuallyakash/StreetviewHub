...
class LocationController extends Controller
{
    ...
    public function storeFavourite($locationName, $latitude, $longitude, $panoId, $panoHeading, $panoPitch, $panoZoom)
    {
        ...
        if ($favourite->count() == 0) {
            ...
            // Downloading Image
            $filepath = 'https://maps.googleapis.com/maps/api/streetview?size=640x480&pano=' . $panoId . '&fov=' . $zoomLevel . '&heading=' . $panoHeading . '&pitch=' . $panoPitch . '&key=' . config('services.gmaps_key');
    ...
    public function favouriteDetails(Request $request)
    {
        ...
        \App\Jobs\TumblrPostEyeshot::dispatch( $location->first() )->delay( now()->addMinutes(11) ); // Publish
